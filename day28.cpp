...
int main(void) {
  ...
  for (int N_itr = 0; N_itr < N; N_itr++) {
    ...
    if (regex_match(emailID, regex("[a-z.]+@gmail.com"))) {
      namesValidated.push_back(firstName);
    }
  }

  sort(namesValidated.begin(), namesValidated.end(), stringComparator);

  for (string name : namesValidated) {
    cout << name << endl;
  }
